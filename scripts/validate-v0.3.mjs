...
if (rootPackage.version !== "0.3.0") failures.push(`root package version is ${rootPackage.version}`);
const dashboardSource = fs.readFileSync(path.join(appRoot, "InteractiveDashboard.tsx"), "utf8");
if (!dashboardSource.includes('NHL_DASHBOARD_VERSION = "0.3.0"')) failures.push("dashboard version constant is not 0.3.0");
if (!dashboardSource.includes('import "./v0.3-polish.css"')) failures.push("v0.3 polish stylesheet is not imported");

const polish = fs.readFileSync(path.join(appRoot, "v0.3-polish.css"), "utf8");
for (const required of [
  ".hd-figma-icon",
  "SF Pro Text",
  "SF Pro Display",
  "width: clamp(250px, 24vw, 300px)",
  "font-size: var(--hd-header-size)",
  "--hd-bg: #e9eff6",
]) {
  if (!polish.includes(required)) failures.push(`v0.3 stylesheet missing token: ${required}`);
}
...
const requiredFiles = [
  ...
  "artifacts/mockup-sandbox/src/components/mockups/hockey-dashboard/v0.3-polish.css",
];
