...
if (rootPackage.version !== "0.5.0") failures.push(`root package version is ${rootPackage.version}`);
const dashboardSource = fs.readFileSync(path.join(appRoot, "InteractiveDashboard.tsx"), "utf8");
if (!dashboardSource.includes('NHL_DASHBOARD_VERSION = "0.5.0"')) failures.push("dashboard version constant is not 0.5.0");
if (!dashboardSource.includes('import "./v0.5-polish.css"')) failures.push("v0.5 polish stylesheet is not imported");
...
const documentationText = ["README.md", "README-V0.5.md", "docs/V0.5_CHANGELOG.md"]
...
for (const required of ["setTimeout", "560", "nhl-dashboard-layout-v0.5", "back 10 sec", "sim.skip(-10)", "useNhlBackendFrame", "layoutForStatPreferences", "ConnectedWidget"]) {
...
const requiredFiles = [
  ...
  "artifacts/mockup-sandbox/src/components/mockups/hockey-dashboard/v0.5-polish.css",
  "artifacts/mockup-sandbox/src/components/mockups/hockey-dashboard/data/nhlBackend.ts",
  "artifacts/mockup-sandbox/src/components/mockups/hockey-dashboard/widgets/ConnectedWidgets.tsx",
];
